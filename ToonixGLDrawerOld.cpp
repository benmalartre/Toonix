...
void TXGLDrawer::DrawCreaseVertices()
{
	std::vector<TXVertex*>::iterator it = _geom->_vertices.begin();
	//draw point
	glColor3f(1,.5,0);
	glPointSize(4);
	glBegin(GL_POINTS);
	for(;it<_geom->_vertices.end();it++)
	{
		if((*it)->_iscrease)glVertex3f((*it)->_pos.GetX(),(*it)->_pos.GetY(),(*it)->_pos.GetZ());
	}
	glPointSize(1);
	glEnd();
}
...
void ToonixGLDrawer_Execute( XSI::CRef in_pSequencerContext, LPVOID *in_pUserData )
{
	...
	if(_drawer._camera == NULL || _drawer._geom == NULL)
		return;
